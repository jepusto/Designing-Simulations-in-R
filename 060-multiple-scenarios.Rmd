...
The `bundle_sim()` function from `simhelpers` will create such a function for us, by combining a performance calculation function with the data-generating and data-analysis functions:\index{example!heteroskedastic ANOVA (Welch)}\index{bundle_sim()}\index{package!simhelpers}
...
As an alternative approach, the `evaluate_by_row()` function from the `simhelpers` package accomplishes the same thing as the `pmap()` calculations inside the `mutate()` step of the above code. Its syntax is a bit more concise:\index{evaluate_by_row()}
