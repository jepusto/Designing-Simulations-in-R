...
Following this structure, we first generate the ability parameters as they are simply standard normal:
```{r}
thetas <- rnorm(N)
```
...
Here is our code from the above thinking:
```{r}
...
1. Modify the Welch simulation's `generate_ANOVA_data()` function from Chapter \@ref(Welch-DGP) to generate data from shifted-and-scaled $t$-distributions rather than from normal distributions. Include the degrees of freedom as an input argument.
...
With new function in hand, simulate a dataset with low degrees of freedom and plot it to see if you see a few outliers.
...
Your new function could simply call the first one, or you can make it entirely stand-alone, your choice.

4. Write some test code to verify the means, variances, and correlations are as specified.

5. Consider target means of $\mathbb{E}(C_1) = \mathbb{E}(C_2) = 10$ and $\mathbb{V}(C_1) = \mathbb{V}(C_2) = 15$. What are the minimum and maximum possible correlations between $C_1$ and $C_2$? 
...
$$
You can do this via the `mvrnorm()` function in the `MASS` package.
...
Following part 5 of Exercise \@ref(BVNB1), use $\mu_1 = \mu_2 = 10$ and $var_1 = var_2 = 15$.
...
2. Sometimes it is wise to prevent nonsensical values. For example, what happens if you put in `alpha = 1` or give an `alpha` larger than 1?  Does it generate data?  It can be useful to put in some error-catching code to prevent such values from being used. For example, you can have `stopifnot( n_min > 0 )` at the top of your function. Implement this error-catching code and check that it works. 

3. The code also rounds `alpha * n_bar`--but what happens if `alpha * n_bar` is not an integer? What will happen when this happens? Is this ok, or possibly a problem?  Explain why or why not.

4. In the `gen_cluster_RCT()` function, there is the following line of code to generate the number of individuals per site.
...
When applying IRT models to actual test data, an analyst will usually be trying to estimate at least some of these parameters.
In particular, they might be fitting the model to _scoring_ the students by estimating their latent ability parameters or fitting a model to _calibrate_ the items by estimating the item characteristics. 
But each time we generate a new dataset with `r_3PL_IRT()`, those latent parameters will change.

In order to understand how well an estimation procedure is performing on data generated by our function, we will need to keep track of these randomly generated parameter values, even though those values will not be known in real data analysis contexts.
...
To make a list in R, you can simply do this:
```{r, eval=FALSE}
res <- list( data = dat, item_parameters = item_dat )
return( res )
```
