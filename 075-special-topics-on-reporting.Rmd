...
Using the prior chapter as a guide, we next recreate some of the plots to show how MCSE\index{Monte Carlo standard error} can distort the picture of what is going on.
...
In the top row, second plot from left, we see that the three estimators are co-dependent: they all react similarly to the same datasets.
...
We next continue the running example of the prior section and fit such a model, taking advantage of the fact that estimated bias is simply the average of the error across replicates.
...
In particular, in Section \@ref(capturing-errors-and-warnings) we illustrated error trapping on the multilevel model for the cluster RCT experiment by revising our `analysis_MLM()` method to a `analysis_MLM_safe()` method, which gave output from the MLM method that looked like this:
