...
## How to analyze simulation results with regression
...
sres_f$ICC_10 = sres_f$ICC * 5 # rescale ICC to be on a 5 point scale
models <- map( meth, function(m) {
  M <- lm( log(SE) ~ (n_bar + J + size_coef + ICC_10 + alpha)^2, 
      data = sres_f %>% filter( method == m ) )
  tidy( M ) %>%
...
## Finding important factors with regression trees
