...
```
The first line specifies that we are creating a function that takes inputs `N` and `mn`. These are called the _parameters_, _inputs_, or _arguments_ of the function. The remaining lines inside the curly brackets is called the _body_ of the function. These lines specify the instructions to follow in transforming the inputs into an output:
