...
For example, say you had a function `simulate()`, like this:
```{r}
...
For example, let us revise the `simulate()` function from above, like this:
...
This means a user can run a basic simulation simply by calling `simulate()` without parameters, and it would perform 1000 trials using a binomial model, and not set a seed first.

Once we have our function with defaults, you can then just change what you want, e.g.,:
```{r}
# Customized simulation with different model and number of trials
