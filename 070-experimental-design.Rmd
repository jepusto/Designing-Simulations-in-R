...
With multifactor experiments, it is easy to end up running a lot of scenarios!
...
For example, suppose you were initially planning on doing 1000 simulations per scenario, but then you realize there is an additional factor that you need to incorporate.
...
To execute the simulation, we will call our simulation function, passing to the function all the parameter values in a given row.
...
These variants automatically stack or convert the list of things returned into a tidier collection (for `_dbl` it will convert to a vector of numbers, for `_dfr` it will stack the results to make a large dataset, assuming each thing returned is a little dataset).
...
These will also be passed to each function call, but will be held constant across all calls.
...
After calling `pmap()` (or `evaluate_by_row()`) and stacking the results, we end up with a dataset containing all the simulation conditions, with the columns consisting of the simulation factors and measured performance outcomes.
This table of performance measures is ideally all we need to conduct further analysis and write up the results.
...
Calculating performance measures after running the simulation, as we just did, leads to the same set of results as doing it inside the simulation driver function.^[In fact, if we use the same seed, we should obtain _exactly_ the same results.]
...
The overall workflow begins by identifying the factors and levels to explore (which we store as a dataset of all the combinations of the factors desired).
...
The key question is then how to explore this full set of results.
The next chapters dive into how to take on this final task.

TO RESOLVE ABOVE AND BELOW
...
>>>>>>> 86b9448 (Little edits to experimental design chapter.)
