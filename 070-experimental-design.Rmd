...
In particular, if `size_coef` is nonzero, but `alpha` is 0, then the cluster size will not impact the cluster average ATE as there is no variation in cluster size.
We could drop those factors, which would make our simulation no longer fully crossed.
This would save us simulation runs---in essence we are running the same simulation for `alpha=0` two times, once for each `size_coef` value, for each level of `ICC`, `n_bar` and `J`.
...
### Selecting the number of factor levels
...
## Running the multifactor simulation
