...
Chapter @sec-presentation-of-results provided some examples of using regression and ANOVA on a set of simulation results to summarize overall patterns across scenarios.
...
As our first in depth example, we walk through the analysis that produces the final ANOVA summary table for the biserial correlation example in Chapter @sec-presentation-of-results.
...
As shown in Chapter @sec-presentation-of-results, we use ANOVA to get a sense of the major sources of variation in the simulation results (e.g., identifying which factors have negligible/minor influence on the bias of an estimator).
...
The eta table in Chapter @sec-presentation-of-results is a summary of this anova table, which we generate as follows:
...
```{r fig-biserial-N, echo=FALSE, message=FALSE, fig.width = 10, fig.height = 5, out.width="100%", fig.cap = "Bias of the biserial correlation estimate in an extreme groups design as a function of inverse sample size, cut-off thresholds, and cut-off types, for selected values of $\\rho$."}
...
To get an initial sense of how bias relates to sample size, we created Figure @fig-biserial-N, which follows a similar layout to Figure @fig-biserial-correlation but shows the inverse sample size on the horizontal axis of each plot, with different values of the correlation parameter corresponding to different colors and line types.
For clarity, the plot shows only a small subset of the correlation levels in the full design.
Figure @fig-biserial-N strongly suggests that the bias is inverse proportional to the sample size. If we let $B_{pqrs}$ be the estimated bias for cut-off level $p$, cut-off type $q$, true correlation level $r$, and sample size level $s$ and let $N_s$ be the actual value of sample size level $s$, we posit that
$$
...
```{r fig-biserial-beta, echo=FALSE, message=FALSE, fig.width = 10, fig.height = 5, out.width="100%", fig.cap = "$N$ times the bias of the biserial correlation estimate in an extreme groups design as a function of $\\rho$, cut-off thresholds, and cut-off types."}
...
Figure @fig-biserial-beta plots $N_s \times B_{pqrs}$, following the same layout as our original Figure @fig-biserial-correlation.
...
We could also create a more succinct summary of the bias relationships by plotting $\beta_{pqr}$ for varying cut-off values, cut-off types, and correlation parameters, as in Figure @fig-biserial-beta-summary-2.

```{r fig-biserial-beta-summary-2, echo=FALSE, message=FALSE, fig.width = 7, fig.height = 3, out.width="100%", fig.cap = "$N$ times the bias of the biserial correlation estimate in an extreme groups design as a function of $\\rho$, cut-off thresholds, and cut-off types."}
...
## Analyzing results with few iterations per scenario {#sec-few-iterations}
...
## Multilevel Meta Regressions {#sec-multilevel-meta-regression}
...
In Section @sec-error-handling we saw how to trap such warnings or errors.
In particular, in Section @sec-capturing-errors-and-warnings we illustrated error trapping on the multilevel model for the cluster RCT experiment by revising our `analysis_MLM()` method to an `analysis_MLM_safe()` method, which gave output from the MLM method that looked like this:
