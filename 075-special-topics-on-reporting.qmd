...
@sec-presentation-of-results provided some examples of using regression and ANOVA on a set of simulation results to summarize overall patterns across scenarios.
...
As our first in depth example, we walk through the analysis that produces the final ANOVA summary table for the biserial correlation example in @sec-presentation-of-results.
...
As shown in @sec-presentation-of-results, we use ANOVA to get a sense of the major sources of variation in the simulation results (e.g., identifying which factors have negligible/minor influence on the bias of an estimator).
...
The eta table in @sec-presentation-of-results is a summary of this anova table, which we generate as follows:
...
To get an initial sense of how bias relates to sample size, we created @fig-biserial-N, which follows a similar layout to @fig-biserial-correlation but shows the inverse sample size on the horizontal axis of each plot, with different values of the correlation parameter corresponding to different colors and line types.
For clarity, the plot shows only a small subset of the correlation levels in the full design.
@fig-biserial-N strongly suggests that the bias is inverse proportional to the sample size. If we let $B_{pqrs}$ be the estimated bias for cut-off level $p$, cut-off type $q$, true correlation level $r$, and sample size level $s$ and let $N_s$ be the actual value of sample size level $s$, we posit that
$$
...
@fig-biserial-beta plots $N_s \times B_{pqrs}$, following the same layout as our original @fig-biserial-correlation.
...
We could also create a more succinct summary of the bias relationships by plotting $\beta_{pqr}$ for varying cut-off values, cut-off types, and correlation parameters, as in @fig-biserial-beta-summary-2.
...
In @sec-error-handling we saw how to trap such warnings or errors.
In particular, in @sec-capturing-errors-and-warnings we illustrated error trapping on the multilevel model for the cluster RCT experiment by revising our `analysis_MLM()` method to an `analysis_MLM_safe()` method, which gave output from the MLM method that looked like this:
