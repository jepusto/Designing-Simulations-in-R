...
This example captures all the major steps of a simulation study\index{simulation study}, which we outlined at the start of Chapter @sec-introduction. 
...
4. Using the prior problem, re-run the simulations to obtain a data frame with each row being a simulation scenario and columns of sample size, estimated coverage, low end of the estimate's confidence interval, high end of the interval, and average confidence interval length.  You will likely want to use `map()` and then `bind_rows()` on your list of results; see Chapter @sec-repeating-oneself for more information about these techniques.
