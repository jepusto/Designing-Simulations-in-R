...
In Chapter @sec-simulation-structure, we described the general structure of basic simulations as following four steps: generate, analyze, repeat, and summarize, with each step represented by its own function or set of code.
...
For example, in Chapter @sec-t-test-simulation, we looked at how the coverage rate of a confidence interval for the mean of an exponential distribution changed as a function of the sample size.
...
To demonstrate, we revisit the case study on heteroskedastic analysis of variance, as studied by @brown1974SmallSampleBehavior and developed in Chapter @sec-case-ANOVA.\index{example!heteroskedastic ANOVA (Welch)}
...
Back in Chapter @sec-case-ANOVA, before we knew much, we developed a function, `generate_ANOVA_data()`, to generate data for a heteroskedastic ANOVA scenario, and a function, `ANOVA_Welch_F()`, to analyze the resulting data.
...
We next create an initial simulation driver by combining `generate_ANOVA_new()` with the data-analysis function we created in Section @sec-ANOVA-hypothesis-testing-function:
```{r}
...
sims
```
Using what we learned in Chapter @sec-performance-measures, we can calculate power as so:
```{r}
...
For a single factor simulation, making our schedule of simulations to run is easy. This step will get more involved with multi-factor simulations, as described in Section @sec-simulating-across-multiple-factors.
...
We first examined this data-generating model in Section @sec-BVPois-example, implementing it in the function `r_bivariate_Poisson()`.
...
In Section @sec-estimation-functions, we developed a function called `r_and_z()` for computing confidence intervals for Pearson's correlation using Fisher's $z$ transformation, and in Section @sec-assessing-confidence-intervals, we wrote a function called `evaluate_CIs()` for evaluating confidence interval coverage and average width. 
...
Another advantage is that it is easy to run the calculations in parallel; see Chapter @sec-parallel-processing.
...
We first ensure we have a directory by making one via `dir.create()` (see Chapter @sec-saving-files for more on files):
...
The following code runs the same experiment as in Section @sec-using-pmap-to-run-multifactor-simulations, but stores the individual replications:
...
@brown1974SmallSampleBehavior evaluated the power of the ANOVA $F$ and Welch test under twenty different conditions, varying in the number of groups, sample sizes, and degree of heteroskedasticity (see Table [5.2](case-ANOVA.html#tab:BF-Scenarios) of Chapter @sec-case-ANOVA). 
...
### Comparing the trimmed mean, median and mean {#exr-trimmed-mean}
