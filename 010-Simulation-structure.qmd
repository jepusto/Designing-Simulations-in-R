...
Monte Carlo\index{Monte Carlo simulation} simulation is a very flexible tool that researchers use to study a vast array of different models and topics. 
...
Finally, we show how the tidy, modular simulation strategy informs the structure and organization of code for a simulation study, walking through basic code skeletons (cf. @sec-function-skeletons) for each of the steps in a single-scenario simulation.
...
<!-- : Steps in the Simulation Process {#tbl-simulation} -->
...
It is apparent from Table @tbl-simulation that writing a simulation in R involves a large space of possibilities and will requiring making a bunch of decisions.
...
We already saw this in Chapter @sec-t-test-simulation, where we modified our initial data-generating process to use a geometric distribution rather than a normal distribution.
...
The principles of tidy simulation imply that code for a simulation study should usually follow the same broad outline and organization of Table @tbl-simulation, with custom functions for each step in process.
...
The `simhelpers` package\index{package!simhelpers} includes the function `create_skeleton()`,\index{create\_skeleton()} which will open a new R script that contains a template for a simulation study, with sections corresponding to each component:
```{r eval=FALSE}
...
We discuss the inputs and form of the data-generating function further in Chapter @sec-data-generating-processes.
...
This might involve generating a confidence interval, as in the example from Chapter @sec-t-test-simulation.
...
We discuss the form and content of the data analysis function further in Chapter @sec-estimation-procedures.
...
[^replicate]: In the example from Chapter @sec-t-test-simulation, we used the `replicate()` function from base R to repeat the process of generating and analyzing data. 
...
We go into further detail about how to approach running the simulation process in Chapter @sec-running-the-simulation-process.
Among other things, we will illustrate how to use the `bundle_sim()` function from the `simhelpers` package to automate the process of coding this step, thereby avoiding the need to write a `one_run()` helper function.\index{bundle\_sim()}\index{package!simhelpers}
...
We might want to know if a confidence interval captures the true parameter the right proportion of the time, as in the simulation from Chapter @sec-t-test-simulation.
...
We discuss the specifics of different performance measures and assessment of Monte Carlo error in Chapter @sec-performance-measures.

### Multifactor simulations

Thus far, we have sketched out the structure of a modular, tidy simulation for a single context.\index{tidy design}
...
We will discuss multiple-scenario simulations in Part III (starting with Chapter @sec-simulating-multiple-scenarios), after we more fully develop the core concepts and techniques involved in simulating a single context.
