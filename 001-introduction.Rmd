...
and even to uncertainty in statistical summaries, as in bootstrapping [@davison1997BootstrapMethodsTheir].
...
1. Create artificial data using random number generators based on a specific statistical model, or more generally, a _data-generating process_.
...
<!-- JEP: Citations for randomization inference and for the bootstrap analogy? -->
...
One example of a simulation investigating questions of finite-sample behavior comes from @longUsingHeteroscedasticityConsistent2000, whose evaluated the performance of heteroskedasticity-robust standard errors (HRSE) in linear regression models. Asymptotic analysis indicates that HRSEs work well in sufficiently large samples [that is, it can be shown that they provide correct assessments of uncertainty when $N$ is infinite; see @White1980heteroskedasticity], but what about in realistic contexts?
...
<!-- JEP: Citations for this? -->
<!-- JEP: This example is not going to be easy for most researchers to follow. What are "numerical (asymptotic based) estimates of performance? Consider either cutting this example or adding more scaffolding.  -->
...
<!-- JEP: Cross-reference to later example? -->
...
The school lottery process creates a natural experiment, based on which researchers can estimate the causal impact of being assigned to one school vs. another. However, a defensible analysis of the process requires knowing the probabilities of school assignment. @abdulkadirouglu2017research conducted such an evaluation using the school lottery process in New York City. 
...
<!-- JEP: Rewrote this in the third person, to describe what Abdulkadiroughlu and colleagues did instead of what we could do. -->
...
<!-- JEP: I would propose that we close this chapter by summarizing the overarching aims of the book and explaining the organization of the subsequent chapters. I've moved the material on R, RStudio, and functions to the next chapter (Programming preliminaries). -->