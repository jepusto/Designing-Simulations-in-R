# Coding Reference {#sec-coding-tidbits}
...
## How to repeat yourself {#sec-more-repeating-oneself}
...
## Default arguments for functions {#sec-default-arguments}
...
## Profiling Code {#sec-profiling-code}
...
For example, we can time how long it takes to analyze some data from our cluster RCT DGP from @sec-case-cluster:\index{example!cluster randomized trial}
...
And we can make a custom plot of time:
```{r}
#| code-fold: show
#| fig-width: 7
#| fig-height: 3
...
ggplot( timings ) +
  aes( n_bar, median, color = as.factor(J) ) + 
  facet_wrap( ~ name ) +
...
## Optimizing code (and why you often shouldn't) {#sec-optimize-code}
...
That warning made, in the next sections we will look at a few optimization efforts applied to the ANOVA example from @sec-case-ANOVA to illustrate some principles of optimization that come up a lot in simulation projects.

### Hand-building functions
In our initial ANOVA simulation (see @sec-case-ANOVA) we used the system-implemented ANOVA.\index{example!heteroskedastic ANOVA (Welch)}
...
We are using data as generated in @sec-data-generating-processes.
...
### Computational efficiency versus simplicity {#sec-comp-efficiency}
