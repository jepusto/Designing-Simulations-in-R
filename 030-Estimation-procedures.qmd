...
We want to know how well a procedure would work when applied in practice, and the data generating processes we described in Chapter @sec-data-generating-processes provides a stand-in for real data.
To revisit our consumer product testing analogy from Chapter @sec-introduction, the data analysis procedure is the product, and the data generating process\index{data-generating process} is the set of trials to which we subject the product.
...
We start by describing the high-level design of an estimation function and illustrate with some simple examples in Section @sec-estimation-functions.
...
We describe how to do this in Section @sec-multiple-estimation-procedures.

In Section @sec-validating-estimation-function, we describe strategies for validating the coded-up estimator before running a full simulation.
...
To allow for this, Section @sec-error-handling demonstrates several methods for handling common computational problems, such as errors or warnings.
...
Would this transformation work well in the face of non-normal data, such as the bivariate Poisson distribution we coded in Chapter @sec-data-generating-processes?\index{example!bivariate Poisson}
...
See Section @sec-about-stopifnot for more.
...
The benefit of stacking will become even more evident once we are working across multiple replications of the simulation process, as we will in Chapter @sec-running-the-simulation-process.
...
In Chapter @sec-case-ANOVA, we wrote a function called `ANOVA_Welch_F()` for computing $p$-values from two different procedures for testing equality of means in a heteroskedastic ANOVA:\index{example!heteroskedastic ANOVA (Welch)}
...
For the bivariate correlation example introduced in Section @sec-estimation-functions, we can check the output of `r_and_z()` against R's built-in `cor.test()` function:
```{r}
...
See Chapter @sec-optimize-code for more discussion of this trade-off.
...
We consider both these problems here, and then revisit the conceptual considerations in Chapter @sec-performance-measures, where we discuss assessing estimator performance.
...
In Chapter @sec-performance-measures, we will delve into the conceptual issues with summarizing the performance of methods that do not always provide an answer.
...
Exercise @sec-contingent-estimator-processing asks you to implement this approach and further refine this contingent analysis strategy.
...
We turn to procedures for doing so in Chapter @sec-running-the-simulation-process.
...
### More Heteroskedastic ANOVA {#sec-BFFs-forever}
...
1. This exercise continues the running example introduces in Chapter @sec-ANOVA-hypothesis-testing-function. Write a function that implements the Brown-Forsythe F\* test (the BFF\* test!) as described on p. 130 of @brown1974SmallSampleBehavior, using the following code skeleton:
    ```{r}
    BF_F <- function( data ) {
...
3. Add the BFF\* test into the output of `ANOVA_Welch_F()` (see Chapter @sec-ANOVA-hypothesis-testing-function) by calling your `BF_F()` function inside the body of `ANOVA_Welch_F()`.^[Note that Exercise @sec-BF-other-tests had you do this using an R package; see next exercise below for using that package to validate your code.]
...
### Contingent testing {#sec-contingent-testing}
...
### Check the cluster-RCT functions {#sec-cross-check-CRT-estimators}
\index{example!cluster randomized trial}

Section @sec-multiple-estimation-procedures presented functions implementing several different strategies for estimating an average treatment effect from a cluster-randomized trial. 
...
### Extending the cluster-RCT functions {#sec-CRT-ANCOVA-estimators}

Exercise @sec-cluster-RCT-baseline from Chapter @sec-data-generating-processes asked you to extend the data-generating function for the cluster-randomized trial to include generating a student-level covariate, $X$, that is predictive of the outcome. 
Use your modified function to generate a dataset. 

1. Modify the estimation functions from Section @sec-multiple-estimation-procedures to use models that include the covariate as a predictor. Your function should return three estimates: an MLM version (`analysis_MLM_cov()`), an aggregated version (`analysis_agg_cov()`), and an OLS version (`analysis_OLS_cov()`. Wrap all three in a final function `analyze_data_cov()` that applies all three methods to the same dataset and returns a tidy dataframe of results. Test that your functions work as expected.
...
### Contingent estimator processing {#sec-contingent-estimator-processing}

In Section @sec-adapting-for-errors we developed a version of `analysis_MLM()` that fell back on OLS regression in the event that `lmer()` produced any error.
...
2. Revise `analyze_data()` to use your new version of `analysis_MLM_contingent()`. The revised function will now sometimes return `NA` values for the `MLM` results. To implement the strategy of falling-back on OLS regression, add some code in `analyze_data()` that replaces any `NA` values for the MLM results with corresponding results of `analysis_OLS()`. Test that the function is working as expected (in this case, if the MLM fails, the final result would have the same estimates for MLM and OLS as two separate rows). Compared to having  `analysis_MLM_contingent()` run OLS internally, writing code this way improves efficiency because we do not fit the OLS regression multiple times on the same data; such an improvement could make a simulation faster to run.

### Estimating 3-parameter item response theory models {#sec-IRT-3PL-estimation}
\index{item response theory}

Exercise @sec-IRT-DGP-parameters asked you to write a data-generating function for the 3-parameter IRT model described in described in Section @sec-three-parameter-IRT. 
...
2. Add an option to the function to allow the user to specify a shared known value `gamma` for the $\gamma_m$. Your function would then assume all items had that value if `gamma` is not `NULL`, but would estimate the $\gamma_m$ if `gamma = NULL`.
...
4. Write a function or set of functions that use the [`{mirt}`](https://cran.r-project.org/package=mirt) package to estimate and return the student-level ability estimates $\theta_1,...,\theta_N$ along with their standard errors, given a test dataset.


### Meta-regression with selective reporting {#sec-Vevea-Hedges-estimation}
\index{example!meta-regression}

Exercise @sec-Vevea-Hedges-DGP asked you to write a data-generating function for the @vevea1995general selection model. 
