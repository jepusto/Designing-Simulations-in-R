...
For estimation functions that involve multi-step procedures or novel methods, other approaches to verification may be needed, which rely more on available statistical theory.
...
Such a test is even more useful here because `r_and_z()` uses our own implementation of the confidence interval calculations, rather than relying on R's built-in functions as we did with `ANOVA_Welch_F()`.


### Checking novel procedures

Simulations are usually an integral part of projects to develop novel statistical methods. 
Such projects present a puzzle: if an estimation procedure truly is new, how do you check that your code is correct?
...
First, we test an algebraic property. 
With each of the three methods, the treatment effect estimator is a difference between the weighted average of the outcomes from students in each treatment condition; the only difference between the estimators is in what weights are used.
...
If there are equal numbers of schools assigned to both conditions, then the standard error from the multilevel model will also be identical to the other standard errors.
...
only partial tests: the function could still be incorrect for scenarios not examined.
