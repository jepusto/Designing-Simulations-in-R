...
In our framework we are generally investigating how well some approach to analyzing data works.
In the prior chapter we made data.
Now we need to analyze it.

Before we discuss the how, some terminology.
A _data analysis procedure_ is a set of steps for analyzing a dataset.
This might be, for example, fitting a multilevel model and then generating a confidence interval for a specific coefficient, running a variable screening step and then fitting a random forest to the remaining selected variables, or dynamically selecting a coarsening level and then making a heatmap.
To keep some focus, we will discuss _estimation procedures_ as a special case of the broader class of data analysis.
In particular, we will discuss these procedures as _estimators_.

Usually, our estimator is our central object of study: we want to know how well it works, and the data generating process is simply the given context for our evaluation.
The estimator is our subject, and the data generating process is, if you will, the test we are giving our subject.

Depending on the research question, our estimator might be very simple---as simple as just computing a sample correlation---or it might involve a combination of several components. 
For example, our "estimator" might entail first computing a diagnostic test for heteroskedasticity and then, depending on the outcome of the test, applying either a conventional formula or a heteroskedasticity-robust formula for standard errors.
As another example, our estimator might first conduct multiple imputation to address missingness on key variables, then fit a statistical model, and then predict values based on the model.

In this chapter, we demonstrate how to bundle an estimator, regardless of its complexity, in the form of an R function, which we call an _estimation function_, so that it performance can eventually be evaluated by repeatedly applying them to artificial data.
We start by describing the high-level design of an estimation function, and illustrate with some simple examples. 

In practice, we frequently are actually comparing competing estimators.
In this case, we will have _several_ functions that implement different estimation procedures that we plan to compare.
...
We discuss how to do this in Section \@ref(multiple-estimation-procedures).

Next, we describe strategies for validating the coded-up estimator before running a full simulation in Section \@ref(running-the-simulation-process).
...
For a full simulation, an estimator needs to be reliable, not crashing and giving answers across the wide range of datasets thrown at it.
...
To allow for this, we close this chapter with several methods for handling common computational problems, such as rare errors or occasional warnings.
...
As a first example, say we wanted to evaluate a method for generating a confidence interval for Pearson's sample correlation coefficient when faced with non-normal data.
For bivariate normal data, statistical theory tells us that Fisher's $z$-transformation of Pearson's correlation coefficient, which is equivalent to the hyperbolic arc-tangent function (`atanh()` in R), is very close to normally distributed and its standard error is simply $1 / \sqrt{N - 3}$, and thus independent of the correlation.
...
We then ask: would this work in the face of non-normal data, such as the bivariate Poisson distribution built in Chapter \@ref(data-generating-processes)?
...
For the function to work on this dataset, we first need to rename the relevant variablesm since our function takes a dataframe with two columns `C1` and `C2`:
...
<!--As a second example, we return to the one-way ANOVA setting from Chapter \@ref(case-ANOVA), where we wrote a function called `ANOVA_Welch_F()` for computing $p$-values from two different procedures for testing equality of means in a heteroskedastic ANOVA:
```{r, eval = FALSE, file = "case_study_code/ANOVA_Welch_F.R"}

```

This function actually contains _two_ estimators, which we could then compare in our simulation.
...
Several different procedures might be used to estimate an overall average effect from a clustered experiment, including these three:
...
Our function outputs only the statistics in which we are interested.
...
Giving full names is also preferable to loading packages inside the function itself (e.g., with `require(lmerTest)`) because calling the function does not change which packages are loaded in the global environment.
...
This is fine: we just wrap all the steps in a single estimation function: from the point of view of _using_ the function, it is a single call, no matter how complicated the process inside.
...
For our Welch test function (see Chapter \@ref(case-ANOVA)) we can check the output of `ANOVA_Welch_F()` against the built-in `oneway.test` function.
...
One might ask why bother implementing your own function, if you already have a version implemented in R?
In some cases, you might be able to implement a faster version of a function because you can cut corners such as verifying input data or providing flexible options, which could help with the overall speed of your simulation.
Frequently, however, such gains, while alluring, are not worth the human time on task it would take to rebuild something from scratch.
...
The new has the exact same parameters as the first function.
...
We can, however, double-wrap the function so it can handle both errors and warnings:
```{r}
...
In our analysis method, we then pick apart the pieces and make a dataframe of them (we also add in a generally preferred optimizer, bobyqa, to try and avoid the warnings and errors in the first place):
...
This is why we often want to say _estimation procedure_ rather than _function_: an analysis may be many different steps in a branching structure of analysis.
...
my_safe_function( 7 )
```
Just as `possibly()` we can include `otherwise = NA` to set what result to have when we fail.
...
Note we still get warnings, but any errors are trapped so we did not crash.

We end up with a list 20 long, with each element of our list a pair of the result and error message:
```{r}
length( resu )
resu[[3]]
```
...
M1
```
Note we have a result object that has a result inside it.
...
From the outside world, we still get a nice, tidy table of results that we can easily use.
