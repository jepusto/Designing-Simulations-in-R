...
The `replicate( n, expr, simplify )` method is a base-R method, where you give it a number `n` and an expression `expr` to run that many times. You can set `simplify = FALSE` to get the results as a list, and if you set `simplify = TRUE` then R will try to simplify your results by, as a default, putting it into an array.
...
To read more about `map()`, check out out [Section 21.5 of R for Data Science (1st edition)](https://r4ds.had.co.nz/iteration.html#the-map-functions), which provides a more thourough introduction to mapping.
