...
Broadly, the answer is to think of the performance measures as outcomes of a designed experiment, so that we can bring to bear any and all of the data analysis techniques relevant to analyzing experimental design involving real data.
...
Traditionally, simulation study results have often been presented in big tables.
...
Furthermore, in simulation, exact values for your bias/RMSE/type-I error, or whatever, are not usually of interest. And in fact, we rarely have them due to Monte Carlo\index{Monte Carlo simulation} simulation error.
...
This is the classic type of table you might see in a paper: the table structured as groups of results indexed by one factor, with a second factor that varies within each group.
...
Because of the ordering of rows, it is difficult to see how the number of control units impacts the rate, and understanding the joint relationship between number of treatment and number of control requires extra thinking.

By contrast, a plot of these exact same numbers can make trends much more clear:

```{r, echo=FALSE, fig.width=6, fig.height=3, out.width="100%"}
...
This is an "interaction plot" showing the "interaction" of the factors nT and nC.
...
Seeing this variation more explicitly might be better done with a visualization, as we explore shortly.
...
That said, the table _is_ showing four performance measures, one of which (the $R^2$) is on a different scale than the others; this is hard to do in a single visualiztion.
...
Figures such as this one clearly show how the estimators are similar or diverge.
...
### Example 2: Biserial correlation estimation
...
@pustejovsky2014converting used simulation examined the bias of biserial correlation estimates under the extreme groups design.
...
Figure \@ref(fig:biserial-correlation) reproduces a visualization of the bias of the biserial correlation estimator for a subset of the conditions examined in @pustejovsky2014converting. The correlation parameter, with 96 levels, runs along the horizontal axis and shows a smoothly varying bias curve.
...
```{r biserial-correlation, echo=FALSE, warning=FALSE, message=FALSE, fig.width = 8, fig.height = 4, out.width="100%", fig.cap = "Bias of the biserial correlation estimate in an extreme groups design, for varying sample sizes, cut-off thresholds, and cut-off types. Source: Pustejovsky (2014)."}
...
r_F <- 
  ...
  filter(stat=="r.i" & design=="Extreme Group", p.inv <= 4) %>%
  droplevels()
...
To make this figure, we smoothed the lines with respect to the correlation parameter $(\rho)$ using `geom_smooth()`.
...
This style of plotting, with a bunch of small plots, is called "many small multiples" and is beloved by Edward Tufte, who has written extensively on best practices for information design (see, for example, @tufte1983visual).
