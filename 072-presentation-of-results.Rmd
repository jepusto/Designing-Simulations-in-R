...
Now we immediately see that only if both nC and nT are above 50 do we get our nominal rates.  Even if nC is 500, we are elevated if nT is only 10.  When nT is 2, then increasing nC actually _increases_ the rejection rate, meaning larger samples are worse; this is not obvious from looking at the raw table results. Plots one, tables zero.
...
```{r anova_example2, warning=FALSE, include=FALSE}
...
One of their tables in the supplementary materials (Table S5.2, see [here](https://osf.io/hy73g), page 20) shows the result of this, with each column being a simulation context, and the rows corresponding to factors manipulated within the simulation for that context:
...
*Note: Small = .01, Medium = .06, Large = .14*
