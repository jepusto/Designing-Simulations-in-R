...
In practice, this amounts to repeatidly calling `generate_data()` and then calling `analyze()` on the result.
...
`assess_performance()`'s other input is `model_params`, which includes the true parameter values of the data-generating process. 
