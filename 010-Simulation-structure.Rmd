...
The `simhelpers` package\index{package!simhelpers} includes the function `create_skeleton()`,\index{create_skeleton()} which will open a new R script that contains a template for a simulation study, with sections corresponding to each component:
```{r eval=FALSE}
...
Among other things, we will illustrate how to use the `bundle_sim()` function from the `simhelpers` package to automate the process of coding this step, thereby avoiding the need to write a `one_run()` helper function.\index{bundle_sim()}\index{package!simhelpers}
