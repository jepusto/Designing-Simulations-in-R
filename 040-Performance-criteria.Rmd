...
Performance criteria are metrics that summarize a sampling distribution---formulas for boiling down the thousands of values from the simulation in order to describe how an estimator or data analysis procedure behaves on average if we could repeat the data-generating process an infinite number of times.
...
Different types of data-analysis results produce different types of information, and so the relevant set of performance criteria depends on the type of data analysis result under evaluation.
...
In defining specific criteria, we will use conventional statistical notation for the means, variances, and other moments of the sampling distribution.
Specifically, we will use the expectation operator $\E()$ to denote the mean of a sampling distribution, $\Var()$ to denote the variance of a sampling distribution, and $\Prob()$ to denote probabilities of specific outcomes with respect to the sampling distribution.
...
## Criteria for Point Estimators {#assessing-point-estimators}
...
$$
\widehat{\Bias} = \bar{T} - \theta, (\#eq:bias-estimator)
$$ 
...
In real data analysis, we never know this value, but in a simulation we can obtain it as the standard deviation of our simulation trial estimates.
...
$$ 
\RMSE = \sqrt{\frac{1}{R} \sum_{r = 1}^R \left( T_r - \theta\right)^2 }.  (\#eq:rmse-estimator)
$$
Often people talk about the MSE (Mean Squared Error), which is just the square of RMSE.
Just like the true SE is versus the variance, we find the units of RMSE easier to interpret than the units of MSE.


For absolute assessments of performance, an estimator with low bias, low variance, and thus low RMSE is desired.
In making comparisons of several different estimators, one with lower RMSE is usually preferable to one with higher RMSE; if two estimators have comparable RMSE, then the estimator with lower bias would usually be preferable. 
...
In many simulations, the scale of the outcome is an arbitrary feature of the data-generating process, making the absolute magnitude of performance criteria less meaningful. 
To ease interpretation of performance criteria, it is useful to consider their magnitude relative to the baseline level of variation in the outcome.
...
Doing so puts the bias, median bias, and root mean-squared error on the scale of standard deviation units, which can facilitate interpretation about what constitutes a meaningfully large bias or a meaningful difference in RMSE.

In addition to understanding the scale of these performance criteria, it is also important to recognize that their magnitude depends on the metric of the parameter. 
A non-linear transformation of a parameter can and often will lead to changes in the magnitude of the performance criteria.
...
and so on.
This is a consequence of how the performance criteria are defined. 
One might see this property as a limitation on the utility of using bias and RMSE to measure the performance of an estimator, since these criteria can be quite sensitive to the metric of the parameter.
...
We demonstrate the calculation of performance criteria for the point estimators of average treatment effects in our cluster-RCT example.
...
Given these results, we can assess the bias, standard error, and RMSE of our three different estimators of the ATE.
These performance criteria address the following questions:
...
Here we assess whether our estimates are systematically different from the $\gamma_1$ parameter we used to generate the data (this is the school-level ATE parameter, which we had set to 0.30).
...
There is no indication of major bias for Agg or MLM.
...
Our estimand, by contrast, is the school-level average treatment effect, i.e., the simple average of each school's true impact, which we have set to 0.30.
The Agg and MLM methods target this school-level average effect, putting them in line with our DGP.
If we had instead decided that our target estimand should be the person-level average effect, then we would find that linear regression is unbiased, whereas Agg and MLM as biased. 
This illustrates that it is crucial to think carefully about the appropriate target parameter and to assess performance with respect to a clearly articulated and justified target.
...
For Agg and MLM, the RMSE is the same as the standard error, which makes sense because these estimators are not biased.
For LR, the combination of bias plus increased variability yields a higher RMSE, with the standard error dominating the bias term (note how RMSE and SE are more similar than RMSE and bias).
...
Overall, Agg and MLM have RMSEs around 17% smaller than LR, a consequential difference in accuracy.
...
In order to compute performance characteristics such as bias and RMSE, we would need to _calculate_ the parameter based on the inputs of the data-generating processes.
There are at least three possible ways to accomplish this.

One way to use mathematical distribution theory to compute an implied parameter.
...
However, this approach requires knowledge of distribution theory, and it can get quite complicated and technical.
...
### Less Conventional Performance Criteria

## Criteria for Standard Error Estimators
...
## Criteria for Inferential Procedures (Hypothesis Tests) {#assessing-inferential-procedures}
...
Thus, most simulations will involve evaluating a data analysis procedure on several criteria to arrive at a holistic understanding of its performance.
...
For such aims, we will need to use the performance criteria to understand whether a set of procedures work differently, when and how one is superior to the other, and what factors influence differences in performance.
