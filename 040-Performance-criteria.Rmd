...
$$
and so on.
This is a consequence of how these performance metrics are defined. 
...
Other metrics such as the median bias and the median absolute deviation of $T$ are less sensitive to outliers in the sampling distribution compared to the conventional metrics.
...
set.seed( 404044 )
runs_val <- sim_function( 
  R, n_bar = 30, J = 20, 
  gamma_1 = 0, gamma_2 = 0.2 
)
```
...
In contrast, for evaluating estimated standard errors we have defined metrics in relative terms, calculated as _ratios_ of the target quantity rather than as differences.
