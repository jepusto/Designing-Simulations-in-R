...
For example, Figure \@ref(fig:CRT-ATE-hist) depicts the distribution of average treatment effect estimates from the cluster-randomized experiment simulation we generated in Section \@ref(one-run-reparameterization). 
...
In Figure \@ref(fig:CRT-ATE-hist), the black dashed lines mark the true average treatment effect of 0.3 and the solid lines with the dot mark the means of the estimators.
...
In Figure \@ref(fig:CRT-ATE-hist), for example, we estimated the bias of each estimator by taking the mean of 1000 observations from its sampling distribution.
...
Here we assess whether our estimates are systematically different from the $\gamma_1$ parameter, which we defined in standardized units by setting the standard deviation of the student-level distribution of the outcome equal to one (see Section\@ref(DGP-standardization)).
...
### Robust Performance Measures {#less-conventional-measures}
...
## Measures for Variance Estimators {#measures-for-variance-estimators}
...
\E[ \sqrt{ V } ] \neq \sqrt{ \E[ V ] }.
$$
Variance is also the measure that gives us the bias-variance decomposition of Equation \@ref(eq:RMSE-decomposition).
...
### Assessing SEs for the Cluster RCT Simulation {#assess-cluster-RCT-SEs}
\index{example!cluster randomized trial}
...
In our earlier assessment of the variance estimator (see \@ref(assess-cluster-RCT-SEs)), we also found Linear Regression had lower degrees of freedom, meaning it might have even wider intervals if they were calculated more carefully.
...
See Chapter \@ref(sec:power) for more on power analysis\index{power analysis} for study planning.
...
Recording the $p$-value of the test makes it easy to estimate rejection rates for multiple $\alpha$ levels, since we simply apply Equation \@ref(eq:rejection-rate-estimate) for each value of $\alpha$.
...
We can do so by running a new simulation using the function we wrote in Section \@ref(one-run-reparameterization), but with an average treatment effect of zero (i.e., we make the null hypothesis true).
...
If the estimand of interest is a location parameter, then the bias, variance or RMSE of an estimator will likely not change as $\theta$ changes (see Figure \@ref(fig:absolute-relative), left).
...
This is what is shown at right of Figure \@ref(fig:absolute-relative).
We usually expect this type of pattern to occur for scale parameters, such as standard deviations or standard errors (as we considered in \@ref(measures-for-variance-estimators)). 
...
A generally more feasible route is to evaluate performance for _multiple_ values of the target parameter and then generate a graph such as shown in Figure \@ref(fig:absolute-relative) to understand how performance changes as a function of the target parameter.
...
We have already demonstrated this approach in comparing standard errors and RMSEs for the cluster RCT example (Section \@ref(clusterRCTperformance)), where we used the linear regression estimator as the benchmark.
...
In Section \@ref(less-conventional-measures) we described several alternative performance measures for evaluating point estimators.
...
### Using the Jackknife to obtain MCSEs (for relative variance estimators) {#MCSE-for-relative-variance}
...
### Bootstrapping MCSEs for relative performance measures {#bootstrapping-MCSEs}
...
perf %>%
  ...
  knitr::kable( digits = 2 )
```
We get essentially the same results as what we found in Section \@ref(bootstrapping-MCSEs) using bootstrapping. 
...
Table \@ref(tab:performance-measure-summary) lists the main measures we have introduced for point estimators; each captures a different aspect of performance, and their importance depends on how the estimator will be used.
...
Table: (\#tab:performance-measure-summary) Performance measures for evaluating point estimators
...
Consider the bivariate negative binomial distribution model described in Exercise \@ref(BVNB2).\index{example!bivariate Poisson}
...
### Confidence interval comparison {#cluster-RCT-t-confidence-intervals}
...
The following code generates 100 replications of a simulation of three average treatment effect estimators in a cluster RCT, using a simulation driver function we developed in Section \@ref(one-run-reparameterization) using components described in Sections \@ref(case-cluster) and \@ref(multiple-estimation-procedures).
