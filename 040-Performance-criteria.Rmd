...
It is important to clarify an important point: the _true standard error_ of an estimator $\hat{\tau}$ is the standard deviation of $\hat{\tau}$ across multiple datasets.
...
People generally, when they say "Standard Error" mean _estimated_ Standard Error, ($\widehat{SE}$) which is when one uses the emperical data to estimate $SE$.
...
```{r load_cluster_sim_data, include=FALSE}
