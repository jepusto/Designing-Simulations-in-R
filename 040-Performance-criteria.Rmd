...
Different types of data-analysis results produce different types of information, and so the relevant set of performance metrics depends on the type of data analysis result we are studying.
...
In defining specific metrics, we will use statistical conventions to denote the mean, variance, and other moments of the sampling distribution.
...
We will use $\Q_p(T)$ to denote the $p^{th}$ quantile of a distribution, which is the value $x$ such that $\Prob(T \leq x) = p$. With this notation, the median is equivalent to the 0.5 quantile: $\M(T) = \Q_{0.5}(T)$.
...
Instead, simulations generate _samples_ (usually large samples) from the the sampling distribution, and we can use these to compute _estimates_ of the performance metrics of interest.
...
### Less Conventional Performance metrics {#less-conventional-metrics}
...
For point estimation, we have introduced bias, variance and RMSE as the three core measures of performance.
...
Estimating these metrics will involve calculating sample quantiles of $T_1,...,T_R$, which are functions of the sample ordered from smallest to largest. 
...
(\#eq:median-bias).
$$
An estimator of median bias is computed using the sample median, equal to 
$$
m_T = \begin{cases} 
T_{((R+1)/2)} & \text{if } R \text{ is odd} \\ 
\frac{1}{2}\left(T_{(R/2)} + T_{(R/2+1)}\right) & \text{if } R \text{ is even.}
\end{cases}
(\#eq:sample-median)
$$
...
To compute estimates of the winsorized performance criteria, we substitute sample quantiles $T_{(R/4)}$ and $T_{(3R/4)}$ in place of $\Q_{0.25}(T)$ and $\Q_{0.25}(T)$, respectively, to get estimated thresholds, $\hat{L}_w$ and $\hat{U}_w$, find $\hat{T}_r^{(w)} = \min\{\max\{T_r, \hat{L}_w\}, \hat{U}_w\}$, and compute the sample performance metrics using Equations \@ref(eq:bias-estimator), \@ref(eq:var-estimator), and \@ref(eq:rmse-estimator), but with $\hat{T}_r^{(w)}$ in place of $T_r$.
...
A further robust measure of central tendency uses the $p \times 100\%$-trimmed mean, which ignores the estimates in the lowest and highest $p$-quantiles of the sampling distribution.
...
$$
\widehat{\text{Trimmed-Bias}}(T; p) = \frac{1}{(1 - 2p)R} \sum_{r=pR + 1}^{(1-p)R} T_{(r)} - \theta. (\#eq:sample-trimmed-bias)
$$
...
## Metrics for Variance Estimators
...
\E[ \sqrt{ V } ] \neq \sqrt{ \E[ V ] }.
$$
Variance is also the metric that gives us the bias-variance decomposition of Equation \@ref(eq:RMSE-decomposition). Thus, if we are trying to determine whether MSE is due to instability or systematic bias, operating in this squared space may be preferable.
...
In these cases, the sampling distribution of the variance estimator is a multiple of a $\chi^2$ distribution, with degrees of freedom corresponding to the number of independent observations used to compute the sum of squares. 
...
## Relative or Absolute Metrics? {#sec-relative-performance}

In considering performance metrics for point estimators, we have defined the metrics in terms of differences (bias, median bias) and average deviations (variance and RMSE), all of which are on the scale of the target parameter.
In contrast, for evaluating estimated standard errors we have defined metrics in relative terms, calculated as _proportions_ of the target quantity rather than as differences.
In the latter case, relative metrics are justified because the target quantity (the true degree of uncertainty) is always positive and is usually strongly affected by design parameters of the data-generating process. 
Is it ever reasonable to use relative measures for point estimators? If so, how should we decide whether to use relative or absolute metrics?

Many published simulation studies have used relative performance metrics for evaluating point estimators. For instance, studies might use relative bias or relative RMSE, defined as
$$
\begin{aligned}
...
Unfortunately, their proposed rule-of-thumb is often generalized far beyond the circumstances where it might be defensible, including to problems where it is clearly arbitrary and inappropriate.

A more principled approach to choosing between absolute and relative metrics is to consider how the magnitude of the metric changes across different values of the target parameter $\theta$.
...
The relationship between bias and the target parameter might be similar to Scenario A in \@ref(fig:absolute-relative), where bias is roughly constant across a range of different values of $\theta$.
Focusing on relative metrics in this scenario would lead to a much more complicated story because different values of $\theta$ will produce drastically different values, ranging from nearly unbiased to nearly infinite bias (for $\theta$ very close to zero). 
...
Focusing on relative metrics in this scenario is useful because it leads to a simple story: relative bias is always around 1.12 across all values of $\theta$, even though the raw bias varies considerably. 
...
#| echo: false
#| message: false
#| fig.cap: "Hypothetical relationships between bias and a target parameter $\\theta$. In Scenario A, bias is unrelated to $\\theta$ and absolute bias is a more appropriate metric. In Scenario B, bias is proportional to $\\theta$ and relative bias is a more appropriate metric."
...
For some estimators and data-generating processes, it may be possible to draw on statistical theory to analyze a problem and examine the how bias or variance would be expected to change as a function of $\theta$.
However, many problems are too complex to be tractable.
Another, much more feasible possibility is to evaluate performance for _multiple_ values of the target parameter.
As done in many simulation studies, we can simulate sampling distributions and calculate performance metrics (in raw terms) for several different values of a parameter, selected so that we can distinguish between constant and multiplicative relationships.
...
Another way to define performance metrics in relative terms to by taking the ratio of the performance measure for one estimator over the performance measure for a benchmark estimator.  
We have already demonstrated this approach in calculating performance metrics for the cluster RCT example (Section \@ref(clusterRCTperformance)), where we used the linear regression estimator as the benchmark against which to compare the other estimators.
...
Comparing the performance of one estimator relative to another can be especially useful when examining metrics whose magnitude varies drastically across design parameters.
...
Because these relative performance metrics are inherently comparative, higher or lower ratios could either be due to the behavior of the method of interest (the numerator) or due to the behavior of the benchmark method (the denominator).
Ratio comparisons are also less effective for performance metrics that are on a constrained scale, such as power.
...
Because of these drawbacks, it is prudent to compute and examine performance metrics in absolute terms in addition to examining relative comparisons between methods.
...
An alternative approach is to simply generate a massive dataset---so large that can stand in for the entire data-generating model---and then simply calculate the target parameter of interest in this massive dataset. In the cluster-RCT example, we can apply this strategy by generating data from a very large number of clusters and then simply calculating the true person-average effect across all generated clusters.
...
If we recalculate performance metrics for all of our estimators with respect to the `ATE_person` estimand, the bias and RMSE of our estimators will shift but the standard errors will stay the same as in previous performance calculations using the school-level average effect:
...
The performance metrics we have described are all defined with respect to the sampling distribution of an estimator, or its distribution across an infinite number of replications of the data-generating process.
Of course, simulations will only involve a finite set of replications, based on which we calculate _estimates_ of the performance metrics.
...
For most of the performance metrics, closed-form expressions are available for calculating MCSEs.
For a few of the metrics, we can apply techniques such as the jackknife to calculate reasonable approximations for MCSEs [@boos2015Assessing].

### Conventional metrics for point estimators

For the metrics that we have described for evaluating point estimators, Monte Carlo standard errors can be calculated using conventional formulas.[^estimated-MCSEs] 
...
[^estimated-MCSEs]: To be precise, the formulas that we give are _estimators_ for the Monte Carlo standard errors of the performance metric estimators. Our presentation does not emphasize this point because the performance metrics will usually be estimated using a large number of replications from an independent and identically distributed process, so the distinction between empirical and estimated standard errors will not be consequential.
...
(\#eq:MCSE-var)
$$
The true standard error (the square root of the sampling variance) is estimated as $S_T$. Using a delta method approximation[^delta-method], the MCSE of $S_T$ is
$$
...
$$
MCSE( \widehat{RMSE} ) = \frac{1}{2 \times \widehat{RMSE}} \times \sqrt{\frac{1}{R}\left[S_T^4 (k_T - 1) + 4 S_T^3 g_T\left(\bar{T} - \theta\right) + 4 S_T^2 \left(\bar{T} - \theta\right)^2\right]}.
(\#eq:MCSE-RMSE)
$$

Section \@ref(sec-relative-performance) discussed circumstances where we might prefer to calculate performance metrics in relative rather than absolute terms. 
For metrics that are calculated by dividing a raw metric by the target parameter, the MCSE for the relative metric is simply the MCSE for the raw metric divided by the target parameter. 
...
### Less conventional metrics for point estimators

In Section \@ref(less-conventional-metrics) we described several alternative performance metrics for evaluating point estimators, which are less commonly used but are more robust to outliers compared to measures like bias and variance.
Monte Carlo standard errors for these less conventional measures can be obtained using results from the theory of robust statistics [@Hettmansperger2010robust; @Maronna2006robust].
...
| Criterion               | Definition                                              | Estimator            |
|-------------------------|---------------------------------------------------------|----------------------|
| __Metrics for point estimators__                                                                         |
| Bias                    | $\E(T) - \theta$                                        | $\bar{T} - \theta$   |
| Median bias             | $\M(T) - \theta$                                        | $m_T - \theta$       |
| Variance                | $\E\left[\left(T - \text{E}(T)\right)^2\right]$         | $S_T^2$              |
| Standard error          | $\sqrt{\E\left[\left(T - \text{E}(T)\right)^2\right]}$  | $S_T$                |
| Mean squared error      | $\E\left[\left(T - \theta\right)^2\right]$              | $\left(\bar{T} - \theta\right)^2 + \frac{R - 1}{R}S_T^2$ |
| Root mean squared error | $\sqrt{\E\left[\left(T - \theta\right)^2\right]}$       | $\sqrt{\left(\bar{T} - \theta\right)^2 + \frac{R - 1}{R} S_T^2}$ |
| Median absolute error   | $\M\left[\left|T - \theta\right|\right]$                | $\left[\left|T - \theta\right|\right]_{R/2}$   |
| Relative bias           | $\E(T) / \theta$                                        | $\bar{T} / \theta$   |
| Relative median bias    | $\M(T) / \theta$                                        | $m_T / \theta$       |
| Relative RMSE           | $\sqrt{\E\left[\left(T - \theta\right)^2\right]} / \theta$ | $\frac{\sqrt{\left(\bar{T} - \theta\right)^2 + \frac{R - 1}{R}S_T^2}}{\theta}$ |
