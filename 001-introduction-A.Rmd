...
Simulations are used in many, many different areas of science, ranging from protein folding to climate change modeling to actuarial science, but in this text we will focus on using simulations to learn about statistical techniques.

Within the fields of statistics and quantitative methodology, simulation studies are commonly used to investigate the properties or performance characteristics of a statistical model or method.
When we apply a statistical method to real data, our goal is to learn something about the process by which that data arose---perhaps we're trying to estimate the mean of a population, or test whether there is a difference in the distributions of two variables---under conditions where we do not know the truth _a priori_. In other words, we're trying to _draw an inductive inference_ about a process on the basis of a sample of data. 
In contrast, simulation studies let us _control_ the process that gives rise to the data, such as by specifying the true parameter values of a statistical model. 
If we then apply a statistical method to those artificially generated data, we can evaluate whether the statistical method "worked" in the sense of giving us a correct answer or an accurate estimate of the truth. 

Simulations are a ubiquitous component of methodological research, used to provide evidence about the performance characteristics of existing statistical procedures or novel, newly proposed methods. Although they're a very common tool, we have found that they are often used in a slap-dash or haphazard fashion 
