...
They looked at the different scenarios shown as Table @tbl-BF-Scenarios, varying number of groups, group size, and amount of variation within each group.
...
Our Table @tbl-BF-table1 reports some of their results with respect to Type I error\index{performance measures!Type I error}.
...
For further discussion of the trade-offs, see Chapter @sec-optimize-code, where we do implement these tests by hand and see what kind of speed-ups we can obtain.
...
In the fifth row of Table 1 (Scenario E in our Table @tbl-BF-Scenarios), Brown and Forsythe examine performance for the following parameter values for sample size and population variance:
...
In Chapter @sec-performance-measures, we will look more at how to estimate and control the Monte Carlo\index{Monte Carlo simulation} simulation error in performance measures.
...
## Exercises {#sec-exAnovaExercises}
...
### Other $\alpha$'s {#sec-BF-other-alphas}
...
### Compare results {#sec-BF-compare-results}
...
Try simulating the **power levels** for a couple of sets of parameter values from Table @tbl-BF-power. 
Use 10,000 replications. 
How do your results compare to the results reported in the Table?

### Wide or long? {#sec-BF-wide-long}
...
### Other tests {#sec-BF-other-tests}
...
Re-run the simulation to estimate the type-I error rate of all four tests under Scenarios A and B of Table @tbl-BF-Scenarios.
