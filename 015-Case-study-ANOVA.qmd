...
```{r tbl-BF-Scenarios, echo=FALSE, message = FALSE}
library(kableExtra)
...
## Exercises {#sec-exAnovaExercises .unnumbered}
...
pow %>%
  kbl(
    caption = 'Portion of "Table 2" reproduced from Brown and Forsythe (1974)',
    label = "tbl-BF-power"
  )
```
...
Modify the data analysis function `ANOVA_Welch_F` (or, better yet, `ANOVA_Welch_F_long` from Exercise @sec-BF-wide-long) to also include results from these hypothesis tests.
