...
# Case study: heteroskedastic ANOVA and Welch {#sec-case-ANOVA}
...
They looked at the different scenarios shown as @tbl-BF-Scenarios, varying number of groups, group size, and amount of variation within each group.
...
```{r}
#| echo: false
#| tbl-cap: Simulation scenarios explored by Brown and Forsythe (1974)
#| label: tbl-BF-Scenarios
...
Our @tbl-BF-table1 reports some of their results with respect to Type I error\index{performance measures!Type I error}.
...
#| echo: false
#| label: tbl-BF-table1
#| tbl-cap: 'Portion of "Table 1" reproduced from Brown and Forsythe (1974)'
...
colnames(table_1) <- c("Scenario", rep(c("10%","5%","1%"), times = 4))
table_1 %>%
  tt() %>%
  group_tt(
    j = list("ANOVA F test" = 2:4, "B & F's F* test" = 5:7, "Welch's test" = 8:10, "James' test" = 11:13)
  ) %>%
  style_tt(j = 1, align = "c") %>%
  style_tt(i = 0:11, j = 2:13, align = "r")
```
...
## The data-generating model {#sec-case-anova-DGP}
...
### Now make a function {#sec-Welch-DGP}
...
## The hypothesis testing procedures {#sec-ANOVA-hypothesis-testing-function}
...
For further discussion of the trade-offs, see @sec-optimize-code, where we do implement these tests by hand and see what kind of speed-ups we can obtain.
...
We next call `one_run()` over and over; see Appendix @sec-repeating-oneself for some discussion of options.
...
In the fifth row of Table 1 (Scenario E in our @tbl-BF-Scenarios), Brown and Forsythe examine performance for the following parameter values for sample size and population variance:
...
In @sec-performance-measures, we will look more at how to estimate and control the Monte Carlo\index{Monte Carlo simulation} simulation error in performance measures.

## Concluding thoughts

Our ANOVA example above and our $t$-test example of @sec-t-test-simulation are, when compared, essentially the same.
Each part of the simulation is a distinct block of code, and together we have a modular simulation that can be easily extended to other scenarios or other tests.
In this example we have bundled each part into its own function; these individual components are more easily customized, tested, and reused.
...
The following exercises involve exploring and tweaking the above simulation code we have developed to replicate the results of @brown1974SmallSampleBehavior.

::: {#exr-BF-other-alphas}

### Other $\alpha$'s 

Table 1 from @brown1974SmallSampleBehavior reported rejection rates for $\alpha = .01$ and $\alpha = .10$ in addition to $\alpha = .05$. Calculate the rejection rates of the ANOVA F and Welch tests for all three $\alpha$-levels and compare to the table.

:::

::: {#exr-BF-compare-results}
...
#| echo: false
#| label: tbl-BF-power
#| tbl-cap: 'Portion of "Table 2" reproduced from Brown and Forsythe (1974)'
...
Try simulating the **power levels** for a couple of sets of parameter values from @tbl-BF-power. 
...
Modify the data analysis function `ANOVA_Welch_F` (or, better yet, `ANOVA_Welch_F_long` from @exr-BF-wide-long) to also include results from these hypothesis tests.
Re-run the simulation to estimate the type-I error rate of all four tests under Scenarios A and B of @tbl-BF-Scenarios.
