...
As we saw in Chapter @sec-simulation-structure, the first step of a simulation is creating artificial data based on some process where we know (and can control) the truth. 
...
We have already seen one example of a DGP in the ANOVA example from Chapter @sec-case-ANOVA.\index{example!heteroskedastic ANOVA (Welch)}
...
Table @tbl-real-vs-sim contrasts the role of structural features, covariates, and outcomes in real data analysis versus in simulations.
...
We have already seen an example of a complete DGP function in the case study on one-way ANOVA (see Section @sec-case-anova-DGP).
...
How would we write a DGP function for the bivariate Poisson model? The equations in Section @sec-DGP-examples give us the recipe, so it just a matter of re-expressing them in code.\index{example!bivariate Poisson}
...
Section @sec-CRT-example introduced a basic multilevel model for a cluster-randomized trial.
...
Exercises @sec-IRT-DGP-parameters, @sec-IRT-DGP-checking, and @sec-IRT-DGP-breaking ask you to further explore these questions.
...
### The Welch test on a shifted-and-scaled $t$ distribution {#sec-Welch-t-dgp}
...
1. Modify the Welch simulation's `generate_ANOVA_data()` function from Chapter @sec-Welch-DGP to generate data from shifted-and-scaled $t$-distributions rather than from normal distributions. Include the degrees of freedom as an input argument.\index{example!heteroskedastic ANOVA (Welch)}
...
3. Once you are satisfied that you have a correct DGP function, re-run the Type-I error rate calculations from the prior exercises in Section @sec-exAnovaExercises using a $t$-distribution with 5 degrees of freedom.
Do the results change substantially?

### Plot the bivariate Poisson
\index{example!bivariate Poisson}

In Section @sec-DGP-functions, we provided an example of a DGP function for the bivariate Poisson model.
We demonstrated a plot of data simulated from this function in Section @sec-DGP-plotting.
...
### Check the bivariate Poisson function {#sec-BVP-check}
...
### Add error-catching to the bivariate Poisson function {#sec-BVP-error}

In Section @sec-DGP-examples, we noted that the bivariate Poisson function as we described it can only produce a constrained range of correlations, which a maximum value that depends on the ratio of $\mu_1$ to $\mu_2$. 
...
### A bivariate negative binomial distribution {#sec-BVNB1}
...
1.  Write a DGP function that implements this distribution given the $\alpha_0$, $\alpha_1$, $\alpha_2$, $\beta$, and $\delta$ parameters.  It should have the following form:
...
### Another bivariate negative binomial distribution  {#sec-BVNB2}
\index{example!bivariate Poisson}
...
3.  Use your new function to create a graph showing the population correlation between the observed counts as a function of the input $\rho$.
Following part 5 of Exercise @sec-BVNB1, use $\mu_1 = \mu_2 = 10$ and $\text{var}_1 = \text{var}_2 = 15$.
How does the range of achievable correlations compare to the range found in that problem?


### Plot the data from a cluster-randomized trial {#sec-cluster-RCT-plot}
...
### Checking the Cluster RCT DGP {#sec-cluster-RCT-checks}
...
### More school-level variation {#sec-cluster-RCT-heterogeneity}
...
### Cluster-randomized trial with baseline predictors {#sec-cluster-RCT-baseline}
...
### 3-parameter IRT datasets {#sec-IRT-DGP-parameters}

A challenge that arises with the IRT model described in Section @sec-three-parameter-IRT is that the data are generated using _random parameters_ (the person ability parameters $\theta_1,...,\theta_N$ and item characteristics $\alpha_m, \beta_m, \gamma_m$ for $m = 1,...,M$) that we simulated from auxiliary models.
...
### Check the 3-parameter IRT DGP {#sec-IRT-DGP-checking}

For one of the DGPs you wrote in Exercise @sec-IRT-DGP-parameters, write code to check that the function is working properly. What features of the model will you check? 

### Explore the 3-parameter IRT model {#sec-IRT-DGP-breaking}

Use the modified DGP function(s) you wrote for Exercise @sec-IRT-DGP-parameters to explore the model.
...
1.  What happens to the distribution of total scores if the items cover a broad range of difficulties (i.e., a very high $\tau_\alpha$)? 
...
### Random effects meta-regression {#sec-meta-regression-DGP}
...
### Meta-regression with selective reporting {#sec-Vevea-Hedges-DGP}
...
They assume that primary study results are generated according to the random effects meta-regression model described in Exercise @sec-meta-regression-DGP, but then only a subset of results are observed, where the probability of being included is a function of the result's one-sided $p$-value for the null hypothesis $H_0: \delta \leq 0$ against alternative $H_A: \delta > 0$.
...
1.  Modify your data-generating function from Exercise @sec-meta-regression-DGP to follow the Vevea-Hedges selection model. Ensure that the new data-generating function returns a total of $K$ primary study results.
...
4.  Use the `selmodel()` function from the `metafor` package to estimate the Vevea-Hedges selection model based on one of your simulated datasets. (See Exercise @sec-Vevea-Hedges-estimation for example syntax.) How do the estimates compare to the model parameters you've specified?
