...
```{r}
#| label: fig-ANOVA-density-ridges
#| code-fold: true
...
```{r}
#| label: fig-bivariate-Poisson-scatter
#| code-fold: true
#| echo: !expr knitr::is_html_output()
#| message: false
...
```{r}
#| label: fig-heteroskedastic-qq-plot
#| code-fold: show
#| fig-width: 5
#| fig-height: 4
#| fig-cap: "QQ plot of simulated data from each of four groups"
...
For instance, we can visualize the distribution of the total scores on the test:
```{r}
#| code-fold: true
#| echo: !expr knitr::is_html_output()
#| warning: FALSE
...
1. Modify the Welch simulation's `generate_ANOVA_data()` function from @sec-Welch-DGP to generate data from shifted-and-scaled $t$-distributions rather than from normal distributions. Include the degrees of freedom as an input argument.\index{example!heteroskedastic ANOVA (Welch)}
...
3. Once you are satisfied that you have a correct DGP function, re-run the Type-I error rate calculations from @exr-BF-other-alphas and @exr-BF-compare-results using a $t$-distribution with 5 degrees of freedom.
...
1.  Modify your data-generating function from @exr-meta-regression-DGP to follow the Vevea-Hedges selection model. Ensure that the new data-generating function returns a total of $K$ primary study results.
...
4.  Use the `selmodel()` function from the `metafor` package to estimate the Vevea-Hedges selection model based on one of your simulated datasets. (See @exr-Vevea-Hedges-estimation for example syntax.) How do the estimates compare to the model parameters you've specified?
