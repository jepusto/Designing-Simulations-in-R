...
In Chapter \@ref(parallel-processing), we demonstrate  parallel processing\index{parallel processing} methods that will help you to speed up the computation involved in multifactor simulations. 
...
In this chapter, we describe strategies for organizing the code and work products involved in a multifactor simulation study\index{simulation study}. 
...
Accordingly, we distinguish between two types of `.R` scripts: those that only contain functions that can be used for other purposes and those that carry out numerical calculations.
...
One reason for storing code in individual files and using `source()` is that you can also then include testing code in each of your files to both demonstrate the syntax and check the correctness of your functions.
...
After we debugged or tweaked our estimators, we could immediately re-run our applied analysis to update the results, without worrying about whether the calculations were up-to-date and consistent with the simulation code.
...
Also see Chapters 13 through 15 of @Wickham2023packages.
