...
<!-- LWM: James, please check out this section and see if it feels overly long or is good at making a case for simulation.  How get some energy into it? -->
...
We explore the @brown1974SmallSampleBehavior study in the case study of @sec-case-ANOVA.
...
For example, simulation can account for variation in the size of clusters in a cluster-randomized trial, or for attrition that is related to baseline characteristics, or for complex mediational relationships that are difficult to analyze mathematically.
See @sec-power for further discussion of power analysis and simulation.
...
<!-- LWM: Should we keep both these examples, above? I wish we could turn something like one of these into a new chapter at the end of our too-long book.  :-(. -->
...
Agenic AI and generative AI also make writing and understanding R code a lot easier.
...
We include several case studies throughout the book; these are designed to make the topics under discussion tangible, and are also designed to provide chunks of code that you emulate or even directly copy and use for your own purposes.
...
Part II lays out the core components (generating artificial data, applying analytic procedures, executing the simulations, and analyzing the simulation results) for simulating a single scenario.
It then presents some more involved case studies that illustrate the principles of modular, tidy simulation design.\index{tidy design}
...
Part IV covers technical challenges that are commonly encountered when programming simulations, including managing large projects, error handling, and parallel computing.
Part V covers some extensions and specialized forms of simulation: simulating to calculate power and simulating within a potential outcomes framework.
These specialized applications underscore how simulation can be used to answer a wide variety of questions across a range of contexts, thus connecting back to the broader purposes discussed above.

The book also includes appendices with some further guidance on writing R code and pointers to further resources.
The code central to simulation implementation is provided and discussed throughout.
Some code for formatting specific tables or generating specific figures is not shown; if you want to that code, consult our online version of the textbook.
All the code there is in an "accordion" so you can expand and view it as needed.
Solutions to most of the exercises are available upon request, if you are planning on teaching a course; please contact the authors.
